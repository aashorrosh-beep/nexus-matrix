...
// DOMAIN: STOREFRONT 9 (PET LIFESTYLE BAR)
...
export default function PetLifestyleStorefront() {
  ...
  useEffect(() => {
    ...
    const fetchInventory = async () => {
      try {
        const res = await fetch('http://localhost:4000/api/storefront/Pet_Lifestyle_Bar');
  ...
  return (
    <section className="w-full max-w-7xl mx-auto px-4 py-8 bg-zinc-950 text-zinc-100 min-h-screen">
      <div className="mb-8 border-b border-zinc-800 pb-4">
        ...
        <p className="text-sm text-yellow-500 font-bold uppercase tracking-widest mt-1 animate-pulse">
          Companion Vault Active — Live Gateway Sync
        </p>
      </div>
      ...
      {loading ? (
         <div className="w-full py-24 flex flex-col items-center justify-center border border-zinc-800 rounded bg-zinc-900/50">
            ...
            <p className="text-zinc-400 uppercase tracking-widest text-sm font-mono text-center leading-relaxed">
              ...
              Scanning vault for premium pet nutrition, tactical harnesses, and smart lifestyle accessories.
            </p>
         </div>
      ) : error ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {inventory.map((item) => (
            <div 
              key={item.id} 
              className="group relative border border-zinc-800 bg-zinc-900 p-5 rounded-lg hover:border-yellow-500 transition-all duration-300 flex flex-col justify-between shadow-lg"
            >
              <div>
                <div className="w-full h-52 bg-zinc-800 rounded mb-4 flex items-center justify-center overflow-hidden relative">
                  ...
                  <span className="text-zinc-600 font-mono text-xs uppercase z-20">Companion_Asset_{item.id}</span>
                </div>
                ...
                <div className="mt-4" aria-label={`Only ${item.stock} units remaining`}>
                  <div className="flex justify-between text-xs mb-1 font-mono uppercase tracking-wider">
                    ...
                    <span className={item.stock <= 2 ? 'text-red-500 font-bold' : 'text-yellow-500'}>
                      {item.stock} Left
                    </span>
                  </div>
                  <div className="w-full h-1.5 bg-zinc-800 rounded-full overflow-hidden">
                    <div 
                      className={`h-full rounded-full transition-all duration-500 ${item.stock <= 2 ? 'bg-red-500 animate-pulse shadow-[0_0_8px_rgba(239,68,68,0.6)]' : 'bg-yellow-500'}`} 
              ...
              <div className="mt-6 pt-5 border-t border-zinc-800">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="bg-zinc-950 px-3 py-1.5 rounded text-xs font-mono font-bold text-yellow-400 border border-yellow-900/30">
